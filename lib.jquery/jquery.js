/ *!
* Библиотека jQuery JavaScript v3.6.0 -ajax, -ajax / jsonp, -ajax / load, -ajax / script, -ajax / var / location, -ajax / var / nonce, -ajax / var / rquery, -ajax / xhr , -manipulation / _evalUrl, -deprecated / ajax-event-alias, -effects, -effects / Tween, -effects / animatedSelector
* https://jquery.com/
*
* Включает Sizzle.js
* https://sizzlejs.com/
...
// достаточно, чтобы все такие попытки были защищены блоком try.
    ...
    функция DOMEval (код, узел, документ) {
        ...
        script.text = код;
        if (node) {
            for (я в сохраненных атрибутахScriptAttributes) {
                ...
                // `jQuery.globalEval`, чтобы он мог подделать узел, содержащий одноразовый номер
                // через объект.
...
// незащищенный в другом месте, кажется безопаснее определить global только для этого модуля
    ...
    jQuery.extend ({
        ...
        // Уникален для каждой копии jQuery на странице
    ...
    if (! seed) {
        ...
        if (documentIsHTML) {
            ...
            если (support.qsa &&
                ...
                // В таких случаях мы обходим поведение, добавляя префикс каждого селектора в
...
function createCache () {
    ...
    кеш функций (ключ, значение) {
        ...
        // Используйте (ключ + ""), чтобы избежать столкновения с собственными свойствами прототипа (см. Проблему № 157)
...
setDocument = Sizzle.setDocument = function (node) {
    ...
    // qSa (: focus) сообщает false, если истинно (Chrome 21)
    ...
    if ((support.qsa = rnative.test (document.querySelectorAll))) {
        ...
        assert (function (el) {
            ...
            // Для выбора специально задана пустая строка
            ...
            // Ничего не следует выбирать, если после ^ = или $ = или * = следуют пустые строки
            ...
            // Добавление временного атрибута в документ до того, как выделение сработает
            // вокруг проблемы.
            ...
            // IE8 выдает здесь ошибку и не будет видеть более поздние тесты
            ...
            // Внутренний селектор `# id sibling-combinator` не работает
    ...
    if ((support.matchesSelector = rnative.test ((matches = docElem.matches ||
        ...
        assert (function (el) {
            ...
            // Проверяем, можно ли выполнить matchSelector
	...
	function (a, b) {
		...
		если (сравнить & 1 ||
			...
			if (a == документ || a.ownerDocument == preferredDoc &&
				содержит (предпочтительныйДок, а)) {
				возврат -1;
			}
...
Выражение = Sizzle.selectors = {
	...
	preFilter: {
		"ATTR": функция (совпадение) {
			...
			// Перемещаем заданное значение в соответствие с [3], в кавычках или без кавычек
